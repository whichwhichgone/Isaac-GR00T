...
DEBUG = True
JSON_FILE_PATH = "/liujinxin/dataset/piper/G1/0420_toy_g1/episode_2/data.json"
DEFAULT_MODEL_SERVER_PORT = 5555
IMAGE_SHAPE = (224, 224, 3)
FIELD_DTYPES = {
    "video": {
        "ego_view": np.uint8,
        "left_wrist_view": np.uint8,
        "right_wrist_view": np.uint8,
    },
    "state": {
...
transform = A.Compose(
    [
        ...
        A.RandomCrop(width=360,height=224, p=0.5),
...
def load_image(image_path, image_type):
    ...
    if DEBUG:
        image.save(f"/liujinxin/liyifan/Isaac-GR00T/image_debug/{image_type}.png")
...
def create_app(policy) -> Flask:
    ...
    def predict():
        ...
        try:
            ...
            image_map = {
            "ego_view" : f"{root_dir}/images/front_head/{index:05d}.png",
            "left_wrist_view" : f"{root_dir}/images/left_hand/{index:05d}.png",
            "right_wrist_view" : f"{root_dir}/images/right_hand/{index:05d}.png"}
            ...
            action = _normalize_policy_output(result)
            end_total = time.perf_counter_ns()

            total_ms = (end_total - start_total) / 1e6
            infer_ms = (end_inf - start_inf) / 1e6

            result = np.concatenate([action[0][v] for v in OUTPUT_FIELD], axis=-1).squeeze(0)
